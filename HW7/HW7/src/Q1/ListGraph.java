...
public class ListGraph
   ...
   * @param listGraph
   */
  public ListGraph(ListGraph listGraph){
    super(listGraph.getNumV(),listGraph.isDirected());
    edges = new List[listGraph.getNumV()];
    for (int i = 0; i < getNumV(); i++){
      edges[i] = new LinkedList <> ();
      edges[i].addAll(listGraph.getEdges()[i]);
    }
  ...
  public void remove(Edge e){
    edges[e.getSource()].remove(new Edge(e.getSource(),e.getDest(),e.getWeight()));
    if (!isDirected())
      edges[e.getDest()].remove(new Edge(e.getDest(), e.getSource(), e.getWeight()));
  }
