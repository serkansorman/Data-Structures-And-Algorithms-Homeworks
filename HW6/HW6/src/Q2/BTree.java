...
public class BTree<E extends Comparable<E>> implements SearchTree<E> {
    ...
    private int binarySearch(E item,E[] data,int start,int end){
        ...
        if(end >= start && middle >= 0 && middle < data.length && data[middle] != null ){
        ...
        return findAvailableIndex(data,item);

    }

    private int findAvailableIndex(E[] data,E item){
        //size of data
        int size;
        for(size=0; size<data.length && data[size] != null; ++size);

        //compares data elements to item
        for(int i=0; i<size; ++i)
            if(item.compareTo(data[i]) < 0)
                return i;

        //if item > all data elements
        return size;
